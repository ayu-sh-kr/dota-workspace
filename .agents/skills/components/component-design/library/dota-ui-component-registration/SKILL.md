...
description: Use when a packages/ui/dota-ui component must be exported, registered, or debugged in packages/apps/dota-web. Covers UI package barrels, main.ts external-component registration, build order, and symptoms of an element that never upgrades.
---

# Dota UI Component Registration

Use this skill after a component exists in `packages/ui/dota-ui` and needs to run in `packages/apps/dota-web`. It is an integration checklist, not a guide for local app components or library API design.
