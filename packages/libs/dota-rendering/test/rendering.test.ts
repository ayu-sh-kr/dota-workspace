...
describe('dota-rendering', () => {
  ...
  it('supports removing a dynamic attribute', () => {
    const root = document.createElement('div');
    const instance = render(root, html`<button disabled=${true}>Save</button>`);

    patch(instance, html`<button disabled=${nothing}>Save</button>`);
