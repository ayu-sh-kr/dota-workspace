...
export class MarkdownViewComponent extends BaseElement {
    ...
    render(): string {
        ...
        const themed = this.content ? applyThemeClasses(this.content, this.theme) : '';
        return HTML`
            <style>${buildSelectionStyle(this._uid, this.theme)}</style>
            <div data-md="${this._uid}"
                 class="${this.maxWidth} mx-auto w-full
